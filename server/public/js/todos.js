...
$(function() {
	...
	var AppView = Backbone.View.extend({
		...
		initialize: function() {
			...
			Todos.fetch({
				reset: true
			}); // reset: true にするとaddが発火しなくなり、reset: falseにするとaddが発火する
		},
		...
		render: function() {
			...
			if (Todos.length) {
				this.main.show();
				this.status_bar.html(this.statsTemplate({
					done: done,
					remaining: remaining
				}));
			} else {
		...
		createOnEnter: function(e) {
			...
			// addイベントの発火タイミングを遅らせる（サーバーの応答後にする）
			Todos.create({
				url: this.input.val()
			}, {
				wait: true
